...
function createNativeDevServerMiddleware(projectRoot, {
  ...
  nativeMiddleware.middleware.use(_internal().ManifestHandler.getManifestHandler(projectRoot)).use(_internal().ExpoUpdatesManifestHandler.getManifestHandler(projectRoot)).use('/symbolicate', (0, _symbolicateMiddleware().createSymbolicateMiddleware)({
    projectRoot,
    compiler,
...
async function startAsync(projectRoot, options = {}) {
  ...
  const compiler = (0, _webpack().default)(config); // Create the middleware required for interacting with a native runtime (Expo Go, or Expo Dev Client).
