...
function _default(program) {
  ...
  (0, _applyAsyncAction().applyAsyncActionProjectDir)(program.command('run:ios [path]').description('Run the iOS app binary locally').helpGroup('core').option('--no-install', 'Skip installing dependencies').option('--no-bundler', 'Skip starting the Metro bundler').option('-d, --device [device]', 'Device name or UDID to build the app on').option('-p, --port <port>', 'Port to start the Metro bundler on. Default: 8081').option('--scheme [scheme]', 'Scheme to build').option('--configuration <configuration>', 'Xcode configuration to use. Debug or Release. Default: Debug'), () => Promise.resolve().then(() => _interopRequireWildcard(require('./ios/runIos'))), {
    checkConfig: false
  });
}
